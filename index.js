...
// const uri=`mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.qvo2oe4.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        const productsCollection = client.db('BioSecure').collection('products');
        const categoryCollection = client.db('BioSecure').collection('Category');
        const usersCollection = client.db('BioSecure').collection("users");
        const bookingsCollection = client.db('BioSecure').collection("bookings");
      ...
      //get the buyers bookings
        ...
        app.patch("/payments/done", async (req, res) => {
            ...
            const query1 = { _id: new ObjectId(bookingID) };
            const query2 = { _id: new ObjectId(productID) };
...
app.get('/', async (req, res) => {
    res.send('BioSecure portal is running');

})
app.listen(port, () => console.log(`BioSecure running on ${port}`));